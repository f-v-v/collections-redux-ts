...
const usersCollectionRequested = ():userCollectionActionTypes => {
    return {type: FETCH_USERS_COLLECTION_REQUEST }
};

const usersCollectionLoaded = (collections:IUserCollection[]):userCollectionActionTypes => {
    return {
        type: FETCH_USERS_COLLECTION_SUCCESS,
        payload: collections
    }
};
const usersCollectionError = (error:string):userCollectionActionTypes => {
    return {
        type: FETCH_USERS_COLLECTION_FAILURE,
        payload:error
    }
};
const usersCollectionAdd = (user:IUserCollection):userCollectionActionTypes => {
    return {
        type: ADD_USERS_COLLECTION,
        payload:user
    }
};
const usersCollectionEdit = (userPermission:IUserCollection):userCollectionActionTypes => {
    return {
